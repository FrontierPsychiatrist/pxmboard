...
class cPrivateInboxList extends cPrivateMessageList{
	 ...
	 */
	function _getQuery(){
		$sQuery = "SELECT p_id,p_subject,p_tstmp,u_id,u_nickname,u_highlight,p_tostate FROM pxm_priv_message,pxm_user WHERE ";
	...
	function deleteData(){
		...
		$objDb->executeQuery("UPDATE pxm_priv_message SET p_tostate=".cMessageStates::messageDeleted()." WHERE p_touserid=$this->m_iUserId");
	...
	function countUnread(){
		...
		global $objDb;
		if($objResultSet = &$objDb->executeQuery("SELECT count(*) AS msgcount FROM pxm_priv_message WHERE p_touserid=$this->m_iUserId AND p_tostate=".cMessageStates::messageNew())){
