...
require_once(INCLUDEDIR."/cUserSearchList.php");
...
class cActionUsersearch extends cAction{
	...
	function performAction(){
		...
		if(empty($sNickName)){
			...
			// userlist
			$objUserSearchList = new cUserSearchList($sNickName);
			$objUserSearchList->loadData($this->m_objInputHandler->getIntFormVar("page",TRUE,TRUE,TRUE),$this->m_objConfig->getUserPerPage());

			$this->m_objTemplate->addData($this->m_objConfig->getDataArray(array_merge($arrUser,
																					   array("previd"	=>$objUserSearchList->getPrevPageId(),
																	   						 "nextid"	=>$objUserSearchList->getNextPageId(),
																							 "nickname"	=>urlencode($sNickName)),
																					   $this->getBannerCode($iIdBoard))));

			$this->m_objTemplate->addData($dummy = array("user"=>$objUserSearchList->getDataArray()));
		}
