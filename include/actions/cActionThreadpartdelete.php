 ...
 */
class cActionThreadpartdelete extends cAction{
	...
	function performAction(){
		...
		if($objActiveBoard = &$this->m_objConfig->getActiveBoard()){
			...
			if($objActiveUser = &$this->m_objConfig->getActiveUser() && ($objActiveUser->isAdmin() || $objActiveUser->isModerator($iBoardId))){
				...
				if($objBoardMessage->loadDataById($this->m_objInputHandler->getIntFormVar("id",TRUE,TRUE,TRUE),$iBoardId)){

					$objThread = new cThread();
					if($objThread->loadDataById($objBoardMessage->getThreadId(),$iBoardId)){
						if($objThread->deleteSubThread($objBoardMessage->getId())){
				...
				else $this->m_objTemplate = &$this->_getErrorTemplateObject(new cError(6));	// invalid message id
			}
