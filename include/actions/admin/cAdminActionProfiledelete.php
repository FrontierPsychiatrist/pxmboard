...
class cAdminActionProfiledelete extends cAdminAction{
	...
	function performAction(){
		...
		if($objActiveUser = &$this->m_objConfig->getActiveUser() && $objActiveUser->isAdmin()){
			...
			$this->m_sOutput .= "<h4>delete profile fields</h4>\n";
			if($objProfileConfig->deleteSlots($this->m_objInputHandler->getArrFormVar("del",TRUE,TRUE,TRUE,"trim","dbattributename"))){
				$this->m_sOutput .= "<h3>profile fields deleted</h3><span id=\"e\">you can adjust templates \"userprofile\", \"userprofileform\" and \"userregistration\" for all skins now!</span>";
			}
			else $this->m_sOutput .= "<h3 id=\"e\">could not delete profile fields</h3>";
		}
